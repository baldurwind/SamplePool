...
function p_listen(subject, mode) {
  ...
  // Loop forever as long not joined
	...
	//	alert(sessionId);
		...
		//var uri = pushletURI + '?userId=' + sessionId + '&p_event=listen';
...
function _initURIs() {
	...
	pushletURI = contextPath+'pushlet.srv';
	pushletNetURI = contextPath+'/resource/lib/js-pushlet-net.html';
}
...
function _processControlQueue() {
	...
	if (sessionId != null) {
		...
		//	var uri = controlQueue.dequeue() + '&userId=' + sessionId;
			pushletNet.setControlURI(uri);
		}
	}
