...
public class Subscriber implements Protocol, ConfigDefs {
	...
	//	System.err.println(clientAdapter);
		...
		while (isActive()) {
			...
			for (int i = 0; i < events.length; i++) {
				...
				//System.out.println(events[i].getField(Subcriber.USERID)+" event ");
				if(getSession().getId().equals(events[i].getField(Subcriber.USERID))){
